...
# ------------------------ RANSAC ----------------------------# # Not currently being used. 
...
class RGBDPointCloudReader(Node):
    def __init__(self):
        ...
        # Subscriptions
        self.pcd_sub   = self.create_subscription(PointCloud2, '/points2', self.point_cloud_callback, 10)
        self.color_sub = self.create_subscription(Image, '/combined_mask/masked_color', self.color_callback,10)
        self.depth_sub = self.create_subscription(Image, '/combined_mask/masked_depth', self.depth_callback, 10)
    ...
    def generate_point_cloud(self):
         ...
         rgbd_image = o3d.geometry.RGBDImage.create_from_color_and_depth(color_o3d, depth_o3d, depth_scale = 1000.0, depth_trunc = 0.75, convert_rgb_to_intensity=False)
    ...
    def point_cloud_callback(self, msg):
        ...
        # Printing this: 
        #print("Cloud array = ", cloud_arr)
        # gives this output:
        """
        Cloud array =  {'xyz': array([[-2.9844239 , -1.6777176 ,  2.9891367 ],
       [-2.979413  , -1.6775239 ,  2.9887915 ],
       [-2.9744015 , -1.6773291 ,  2.9884446 ],
       ...,
       [ 0.55910623,  0.31529206,  0.56174594],
       [ 0.55995256,  0.3152748 ,  0.5617152 ],
       [ 0.56079876,  0.31525746,  0.5616843 ]], dtype=float32), 'rgb': array([[191, 115, 192],
       [185,  26, 192],
       [178, 184, 192],
       ...,
       [109, 246,  63],
       [107, 179,  63],
       [105, 109,  63]], dtype=uint8)}
        """
        # Extract XYZ
        ...
        min_distance_threshold = 0.1# Overhead perspective#0.1 # Min dist threshold in meters
        max_distance_threshold = 2# Overhead perspective#0.7 # Max Distance threshold in meters
        ...
        if self.generated_pcd is not None:
            ...
            #o3d.visualization.draw_geometries([filtered_pcd, pcd2])  
            #voxel_size = 0.05
            #source_down, source_fpfh = preprocess_point_cloud(pcd2, voxel_size)
            #target_down, target_fpfh = preprocess_point_cloud(filtered_pcd, voxel_size)
...
"""
This version works for 1 static point cloud and the point cloud from the stream. 
# Can read point cloud stream from ROS2
# Colors don't work. 
import numpy as np
import open3d as o3d
...
class RGBDPointCloudReader(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            PointCloud2, '/depth/color/points', self.point_cloud_callback, 10)
    ...
    def point_cloud_callback(self, msg):
       ...
       [105, 109,  63]], dtype=uint8)}
        ...
        #colors = cloud_arr['rgb']

        #colors_float = cloud_arr['rgb'].astype(np.float64)
        #for color in colors:
        #    print(color)
        #print("------")
        
        #for color in colors_float[:5]:
        #    print(color)
        #print("*********")
        # Extract RGB values assuming they are in RGB8 format
        # Note: Depending on the encoding you may need to change the order
        #olors = np.zeros((len(cloud_arr['rgb']), 3), dtype=np.float64)
        #colors[:,0] = cloud_arr['rgb'][:,0] # Red channel
        #colors[:,1] = cloud_arr['rgb'][:,1] # Green channel
        #colors[:,2] = cloud_arr['rgb'][:,2] # Blue channel
        ...
        pcd2 = o3d.io.read_point_cloud("capture.ply")
